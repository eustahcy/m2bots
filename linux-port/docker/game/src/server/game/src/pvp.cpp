...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	// A bot has no client to answer with a /pvp of its own, so the challenge
	// would hang here unanswered. Leave it for the bot's tick instead
	// (AcceptPlayerBotPvpChallenge). Only a NEW duel reaches this line - the
	// branch above returns as soon as the pair already exists - so the bot's
	// own answering Insert cannot be recorded as a fresh challenge.
