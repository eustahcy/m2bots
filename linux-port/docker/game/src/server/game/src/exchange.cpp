...
bool CExchange::Accept(bool bAccept)
{
	...
	// A Playerbot has no client and will never press Accept, so a trade with one
	// - a companion being handed gear, most of all - would sit open until the
	// player gave up. Accept on its behalf once the human side has accepted.
	// Only the bot's own side is set here: the player still confirms their half,
	// and a trade between two humans is untouched.
	if (m_bAccept && GetCompany() && GetCompany()->GetOwner() &&
			GetCompany()->GetOwner()->GetDesc() &&
			GetCompany()->GetOwner()->GetDesc()->IsBot())
		GetCompany()->m_bAccept = true;
