...
param(
    ...
    [switch]$ThenStart,
    [switch]$Yes,
    # DeployVps only. VpsPassword is used exactly once, to authorise the local
    # deploy key on a VPS that has never seen it - never written to disk, never
    # reused once the key itself is trusted.
    [string]$VpsHost = '',
    [string]$VpsUser = '',
    [string]$VpsPath = '',
    [string]$VpsPassword = ''
...
function Write-Header {
    ...
    Write-Host '  EVEREN - Launcher i aktualizacje' -ForegroundColor Yellow
    Write-Host '========================================================' -ForegroundColor DarkYellow
    Write-Host ''
}
...
function Send-Logs {
    ...
    if (-not $support.UploadUrl) {
        if ($support.ContactUrl) {
            throw "Kanał zgłoszeń jest teraz niedostępny. Utwórz ZIP akcją Logs i wyślij go ręcznie na: $($support.ContactUrl)"
        }
        throw 'Nie skonfigurowano kanału zgłoszeń. Utwórz paczkę akcją Logs (folder support-bundles) i wyślij ją administratorowi swojego serwera ręcznie.'
    }
    $bundle = Create-Logs
...
# --- Wyślij na VPS ----------------------------------------------------------
#
# Kopiuje linux-port\docker (bez .env - ten na serwerze zostaje nietknięty) na
# zdalny VPS przez SSH. Logowanie po kluczu, nigdy po zapamiętanym haśle: ten
# plik trzyma tylko adres, użytkownika i ścieżkę zdalną (.m2vps-deploy.json) -
# hasło z okna GUI żyje wyłącznie w pamięci tego jednego procesu, na czas
# jednorazowej autoryzacji klucza, i nigdy nie trafia na dysk.
$script:vpsConfigPath = Join-Path $serverRoot '.m2vps-deploy.json'

function Get-VpsDeployConfig {
    if (Test-Path -LiteralPath $script:vpsConfigPath -PathType Leaf) {
        try { return (Get-Content -LiteralPath $script:vpsConfigPath -Raw -Encoding UTF8 | ConvertFrom-Json) }
        catch { }
    }
    return [pscustomobject]@{ host = ''; user = 'debian'; remotePath = '/home/debian/metin2-playerbots/linux-port/docker' }
}

function Save-VpsDeployConfig {
    param([Parameter(Mandatory = $true)]$Config)
    [pscustomobject]@{ host = $Config.host; user = $Config.user; remotePath = $Config.remotePath } |
        ConvertTo-Json | Set-Content -LiteralPath $script:vpsConfigPath -Encoding UTF8
}

function Get-VpsDeployKeyPath {
    Join-Path $env:USERPROFILE '.ssh\m2_vps_deploy_ed25519'
}

function Test-VpsDeployKeyAuth {
    param([Parameter(Mandatory = $true)][string]$TargetHost, [Parameter(Mandatory = $true)][string]$User,
          [Parameter(Mandatory = $true)][string]$KeyPath)
    # ssh writing "Permission denied" to stderr is the expected, silent way
    # this probe fails before the key is authorised - not a real error. Under
    # $ErrorActionPreference='Stop' (set at the top of this script), 2>&1 on a
    # native command promotes that stderr line into a terminating exception in
    # Windows PowerShell 5.1, which skipped straight past "return $false" to
    # the top-level catch and printed the raw ssh error as "BŁĄD: ..." before
    # the deploy ever got to ask for a password. Same fix as Stop-Server/
    # Update-Server's docker compose calls: relax locally, restore after.
    $previousPreference = $ErrorActionPreference
    try {
        $ErrorActionPreference = 'Continue'
        $probe = & ssh -i $KeyPath -o BatchMode=yes -o StrictHostKeyChecking=accept-new -o ConnectTimeout=6 "$User@$TargetHost" 'echo m2ok' 2>&1
    }
    finally { $ErrorActionPreference = $previousPreference }
    return ($LASTEXITCODE -eq 0 -and ($probe -join "`n") -match 'm2ok')
}

function Install-VpsDeployKey {
    # A one-shot ssh-copy-id: the password authenticates exactly one command,
    # appending the public key, through OpenSSH's SSH_ASKPASS mechanism rather
    # than an interactive prompt - this runs headless, inside a GUI action with
    # no console to prompt on.
    param([Parameter(Mandatory = $true)][string]$TargetHost, [Parameter(Mandatory = $true)][string]$User,
          [Parameter(Mandatory = $true)][string]$Password, [Parameter(Mandatory = $true)][string]$PublicKeyText)
    $askpass = Join-Path $env:TEMP "m2-vps-askpass-$PID.cmd"
    $savedAskpass = $env:SSH_ASKPASS
    $savedRequire = $env:SSH_ASKPASS_REQUIRE
    try {
        Set-Content -LiteralPath $askpass -Value @("@echo off", "echo $Password") -Encoding ASCII
        $env:SSH_ASKPASS = $askpass
        $env:SSH_ASKPASS_REQUIRE = 'force'
        $remoteCommand = "mkdir -p ~/.ssh && chmod 700 ~/.ssh && grep -qxF '$PublicKeyText' ~/.ssh/authorized_keys 2>/dev/null || echo '$PublicKeyText' >> ~/.ssh/authorized_keys; chmod 600 ~/.ssh/authorized_keys && echo M2KEYOK"
        # Same 2>&1-under-Stop issue as Test-VpsDeployKeyAuth: a warning on
        # stderr (e.g. "Permission added" prompts, host key notices) must not
        # abort this before $result is even read.
        $previousPreference = $ErrorActionPreference
        try {
            $ErrorActionPreference = 'Continue'
            $result = & ssh -o StrictHostKeyChecking=accept-new -o PreferredAuthentications=password -o PubkeyAuthentication=no "$User@$TargetHost" $remoteCommand 2>&1
        }
        finally { $ErrorActionPreference = $previousPreference }
        return ($LASTEXITCODE -eq 0 -and ($result -join "`n") -match 'M2KEYOK')
    }
    finally {
        Remove-Item -LiteralPath $askpass -Force -ErrorAction SilentlyContinue
        if ($savedAskpass) { $env:SSH_ASKPASS = $savedAskpass } else { Remove-Item Env:\SSH_ASKPASS -ErrorAction SilentlyContinue }
        if ($savedRequire) { $env:SSH_ASKPASS_REQUIRE = $savedRequire } else { Remove-Item Env:\SSH_ASKPASS_REQUIRE -ErrorAction SilentlyContinue }
    }
}

function New-PosixZipArchive {
    # Compress-Archive writes backslash path separators in its entries on
    # Windows, which is not the ZIP standard and is not a directory separator
    # to unzip on Linux - it unpacked as one file literally named
    # "tuike\tuike\gamedata\bots.py" instead of nested folders. Built directly
    # on System.IO.Compression instead, with every entry name normalised to
    # forward slashes by hand.
    param(
        [Parameter(Mandatory = $true)][string]$SourceDir,
        [Parameter(Mandatory = $true)][string]$DestZip,
        [string[]]$ExcludeTopLevel = @(),
        [string[]]$ExcludeNames = @('.env')
    )
    ...
    $archive = [IO.Compression.ZipFile]::Open($DestZip, [IO.Compression.ZipArchiveMode]::Create)
    try {
        Get-ChildItem -LiteralPath $sourceFull -Recurse -File -Force | ForEach-Object {
            ...
            if ($ExcludeTopLevel -contains $segments[0]) { return }
            if ($ExcludeNames -contains $_.Name) { return }
            $entryName = $relative -replace '\\', '/'
            [IO.Compression.ZipFileExtensions]::CreateEntryFromFile(
                $archive, $_.FullName, $entryName, [IO.Compression.CompressionLevel]::Optimal) | Out-Null
        }
    }
    finally { $archive.Dispose() }
}

function Deploy-ToVpsAction {
    if (-not (Get-Command ssh -ErrorAction SilentlyContinue) -or -not (Get-Command scp -ErrorAction SilentlyContinue)) {
        throw 'Brak klienta SSH (ssh.exe / scp.exe). W Windows: Ustawienia > Aplikacje > Funkcje opcjonalne > dodaj "OpenSSH Client".'
    }
    $saved = Get-VpsDeployConfig
    $targetHost = if ($VpsHost) { $VpsHost } else { [string]$saved.host }
    $user = if ($VpsUser) { $VpsUser } elseif ([string]$saved.user) { [string]$saved.user } else { 'debian' }
    $remotePath = if ($VpsPath) { $VpsPath } elseif ([string]$saved.remotePath) { [string]$saved.remotePath } else { '/home/debian/metin2-playerbots/linux-port/docker' }
    if (-not $targetHost) { throw 'Brak adresu VPS. Podaj go w oknie WYŚLIJ NA VPS.' }
    Save-VpsDeployConfig -Config ([pscustomobject]@{ host = $targetHost; user = $user; remotePath = $remotePath })

    $keyPath = Get-VpsDeployKeyPath
    if (-not (Test-Path -LiteralPath $keyPath -PathType Leaf)) {
        Write-Host 'Generuję klucz SSH do wdrożeń (tylko za pierwszym razem)...' -ForegroundColor Cyan
        New-Item -ItemType Directory -Path (Split-Path -Parent $keyPath) -Force | Out-Null
        & ssh-keygen -t ed25519 -f $keyPath -N '""' -C 'm2-vps-deploy' -q
        if ($LASTEXITCODE -ne 0 -or -not (Test-Path -LiteralPath $keyPath -PathType Leaf)) {
            throw 'Nie udało się wygenerować klucza SSH (ssh-keygen).'
        }
    }
    $publicKey = (Get-Content -LiteralPath "$keyPath.pub" -Raw).Trim()

    Write-Host "Sprawdzam dostęp do $user@$targetHost..." -ForegroundColor Cyan
    if (-not (Test-VpsDeployKeyAuth -TargetHost $targetHost -User $user -KeyPath $keyPath)) {
        if (-not $VpsPassword) {
            throw "Klucz wdrożeniowy nie jest jeszcze autoryzowany na $targetHost. Podaj hasło w oknie WYŚLIJ NA VPS - użyje go tylko raz, do zapisania klucza, i nigdzie go nie zapisze."
        }
        Write-Host "Autoryzuję klucz wdrożeniowy na $targetHost (jednorazowo, hasłem)..." -ForegroundColor Cyan
        if (-not (Install-VpsDeployKey -TargetHost $targetHost -User $user -Password $VpsPassword -PublicKeyText $publicKey)) {
            throw 'Nie udało się zapisać klucza na serwerze - sprawdź adres, użytkownika i hasło.'
        }
        Start-Sleep -Seconds 1
        if (-not (Test-VpsDeployKeyAuth -TargetHost $targetHost -User $user -KeyPath $keyPath)) {
            throw 'Klucz zapisany, ale logowanie nim wciąż się nie udaje. Spróbuj ponownie.'
        }
        Write-Host 'Klucz autoryzowany. Kolejne wysyłki na ten serwer nie będą już pytać o hasło.' -ForegroundColor Green
    }

    Write-Host 'Pakuję linux-port\docker (bez .env)...' -ForegroundColor Cyan
    $stagingZip = Join-Path $env:TEMP 'm2-vps-deploy.zip'
    New-PosixZipArchive -SourceDir (Join-Path $serverRoot 'linux-port\docker') -DestZip $stagingZip -ExcludeTopLevel @('.claude')
    $sizeMb = [Math]::Round((Get-Item -LiteralPath $stagingZip).Length / 1MB, 1)
    Write-Host "Archiwum gotowe ($sizeMb MB). Wysyłam do $targetHost..." -ForegroundColor Cyan

    $remoteZip = "/tmp/m2-vps-deploy-$(Get-Date -Format 'yyyyMMddHHmmss').zip"
    & scp -i $keyPath -o StrictHostKeyChecking=accept-new $stagingZip "${user}@${targetHost}:${remoteZip}"
    if ($LASTEXITCODE -ne 0) { throw 'Wysyłka archiwum nie powiodła się (scp).' }
    Remove-Item -LiteralPath $stagingZip -Force -ErrorAction SilentlyContinue

    Write-Host "Rozpakowuję na serwerze do $remotePath (bez ruszania tamtejszego .env)..." -ForegroundColor Cyan
    $remoteScript = @"
set -e
sudo mkdir -p '$remotePath'
command -v unzip >/dev/null 2>&1 || (sudo apt-get update -qq && sudo apt-get install -y -qq unzip)
sudo unzip -oq '$remoteZip' -d '$remotePath'
rm -f '$remoteZip'
echo M2DEPLOYOK
"@
    $previousPreference = $ErrorActionPreference
    try {
        $ErrorActionPreference = 'Continue'
        $result = & ssh -i $keyPath "${user}@${targetHost}" $remoteScript 2>&1
    }
    finally { $ErrorActionPreference = $previousPreference }
    Write-Host ($result -join [Environment]::NewLine)
    if ($LASTEXITCODE -ne 0 -or ($result -join "`n") -notmatch 'M2DEPLOYOK') {
        throw 'Rozpakowanie na serwerze nie powiodło się - szczegóły w logu powyżej.'
    }
    Write-Host "Wdrożono do $user@${targetHost}:$remotePath." -ForegroundColor Green
    Write-Host 'Pliki są na miejscu, ale serwer NIE został sam z siebie przebudowany ani zrestartowany - zrób to z panelu VPS (port 9797: Kompiluj, potem Restart) albo ręcznie przez SSH.' -ForegroundColor Yellow
}

function Assert-ManifestConfigured {
    # Everen contacts no update channel by default (see
    # Get-M2DefaultLauncherConfig) - a player who wants this back points
    # Configure-Launcher at a channel Everen actually controls. Without one,
    # every update action says so plainly instead of failing on an empty URL.
    param($Config)
    if ([string]$Config.manifestUrl) { return $true }
    Write-Host 'Aktualizacje nie są skonfigurowane w tym launcherze - Everen nie sprawdza żadnego zewnętrznego źródła samo z siebie.' -ForegroundColor Yellow
    Write-Host 'Jeśli prowadzisz własny kanał aktualizacji, ustaw jego adres w opcji 12 (Konfiguracja launchera).' -ForegroundColor Yellow
    return $false
}
...
function Invoke-Action {
    ...
    switch ($SelectedAction) {
        ...
        'Check' {
            if (-not (Assert-ManifestConfigured $config)) { return }
    ...
    while ($true) {
        ...
        $selected = switch ($choice) {
            ...
            '22' {
                ...
                $vpsConfig = Get-VpsDeployConfig
                $hostInput = Read-Host "Adres VPS [$($vpsConfig.host)]"
                if ($hostInput) { $script:VpsHost = $hostInput } else { $script:VpsHost = [string]$vpsConfig.host }
                $userInput = Read-Host "Użytkownik SSH [$($vpsConfig.user)]"
                if ($userInput) { $script:VpsUser = $userInput } else { $script:VpsUser = [string]$vpsConfig.user }
                $pathInput = Read-Host "Ścieżka docelowa [$($vpsConfig.remotePath)]"
                if ($pathInput) { $script:VpsPath = $pathInput } else { $script:VpsPath = [string]$vpsConfig.remotePath }
                $securePassword = Read-Host 'Hasło SSH (Enter jeśli klucz jest już autoryzowany)' -AsSecureString
                if ($securePassword.Length -gt 0) {
                    $script:VpsPassword = [Runtime.InteropServices.Marshal]::PtrToStringAuto(
