...
# Fallback used when the manifest carries no support block (offline, or an old
# manifest). Empty on purpose: Everen is its own project now, not a channel of
# the upstream Playerbots one, and a launcher that still pointed players at
# somebody else's Discord for support would be a worse answer than none.
$script:M2_DEFAULT_SUPPORT_CONTACT = ''
...
function Get-M2DefaultLauncherConfig {
    ...
    # Everen ships as its own fork, diverged from upstream Playerbots by real
    # server-side changes (gear scoring, market behaviour, the Tuike panel and
    # more) - an update channel still pointed at TieruYT/metin2-playerbots
    # would offer to overwrite that work with someone else's tree the moment a
    # release appeared there. No default manifest is the disconnect: this
    # launcher contacts nothing on its own unless Configure-Launcher is told a
    # URL to use, deliberately, for a channel Everen actually controls.
    $manifest = ''
...
function Get-M2LauncherConfig {
    ...
    # A config saved by a launcher from before Everen disconnected from the
    # upstream Playerbots channel still names it by URL - that is a leftover
    # default, not a choice anybody made, and left alone it would go on
    # contacting TieruYT/metin2-playerbots forever even on an install that
    # updates to this launcher. Reset it to Everen's own (empty) default; any
    # OTHER manifestUrl an operator pointed at deliberately is left exactly as
    # they set it.
    if ([string]$defaults.manifestUrl -like 'https://raw.githubusercontent.com/TieruYT/metin2-playerbots/*') {
        $defaults.manifestUrl = ''
    }
